import Image from "next/image";
import FormContainer from "@/components/FormContainer";
import Pagination from "@/components/Pagination";
import Table from "@/components/Table";
import TableSearch from "@/components/TableSearch";
import prisma from "@/lib/prisma";
import { ITEM_PER_PAGE } from "@/lib/settings";
import { Prisma, FeePayment, Student, FeeCategory } from "@prisma/client";
import { auth } from "@/lib/auth-helpers";
import PaymentModal from "@/components/PaymentModal";

type FeePaymentList = FeePayment & { student: Student; category: FeeCategory };

const FeeListPage = async ({
  searchParams,
}: {
  searchParams: { [key: string]: string | undefined };
}) => {
  const { userId, sessionClaims } = await auth();
  const role = (sessionClaims?.metadata as { role?: string })?.role;

  const columns = [
    {
      header: "Student",
      accessor: "student",
    },
    {
      header: "Fee Type",
      accessor: "category",
    },
    {
      header: "Amount",
      accessor: "amount",
      className: "hidden md:table-cell",
    },
    {
      header: "Date",
      accessor: "date",
      className: "hidden lg:table-cell",
    },
    {
      header: "Status",
      accessor: "status",
      className: "hidden lg:table-cell",
    },
    {
      header: "Actions",
      accessor: "action",
    },
  ];

  const renderRow = (item: FeePaymentList) => (
    <tr
      key={item.id}
      className="border-b border-gray-200 even:bg-slate-50 text-sm hover:bg-plPurpleLight"
    >
      <td className="flex items-center gap-4 p-4">{item.student.name} {item.student.surname}</td>
      <td>{item.category.name}</td>
      <td className="hidden md:table-cell">₹{item.amountPaid}</td>
      <td className="hidden lg:table-cell">{item.paymentDate.toLocaleDateString()}</td>
      <td className="hidden lg:table-cell">
        <span className={`px-2 py-1 rounded-md text-white ${item.status === 'PAID' ? 'bg-green-500' : 'bg-red-500'}`}>
            {item.status}
        </span>
      </td>
      <td>
        <div className="flex items-center gap-2">
          {item.status === "PENDING" && <PaymentModal item={item} />}
          {(role === "admin" || role === "accountant") && (
            <>
              <FormContainer table="feePayment" type="update" data={item} />
              <FormContainer table="feePayment" type="delete" id={item.id} />
            </>
          )}
        </div>
      </td>
    </tr>
  );

  const { page, ...queryParams } = searchParams;
  const p = page ? parseInt(page) : 1;

  const query: Prisma.FeePaymentWhereInput = {};

  if (queryParams) {
    for (const [key, value] of Object.entries(queryParams)) {
      if (value !== undefined) {
        switch (key) {
          case "search":
            query.student = { name: { contains: value } };
            break;
          default:
            break;
        }
      }
    }
  }

  // ROLE CONDITIONS
  switch (role) {
    case "student":
      query.studentId = userId!;
      break;
    case "parent":
      query.student = { parentId: userId! };
      break;
    default:
      break;
  }

  const [data, count] = await prisma.$transaction([
    prisma.feePayment.findMany({
      where: query,
      include: {
        student: true,
        category: true,
      },
      take: ITEM_PER_PAGE,
      skip: ITEM_PER_PAGE * (p - 1),
    }),
    prisma.feePayment.count({ where: query }),
  ]);

  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
      {/* TOP */}
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-lg font-semibold">Fee Collections</h1>
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          <TableSearch />
          <div className="flex items-center gap-4 self-end">
            <button className="w-8 h-8 flex items-center justify-center rounded-full bg-plYellow">
              <Image src="/filter.png" alt="" width={14} height={14} />
            </button>
            <button className="w-8 h-8 flex items-center justify-center rounded-full bg-plYellow">
              <Image src="/sort.png" alt="" width={14} height={14} />
            </button>
            {(role === "admin" || role === "accountant") && (
              <FormContainer table="feePayment" type="create" />
            )}
          </div>
        </div>
      </div>
      {/* LIST */}
      <Table columns={columns} renderRow={renderRow} data={data} />
      {/* PAGINATION */}
      <Pagination page={p} count={count} />
    </div>
  );
};

export default FeeListPage;
